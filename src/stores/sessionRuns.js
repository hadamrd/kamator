...
export const useSessionRunStore = defineStore('sessionRun', {
    ...
    getters: {
        isSessionRunning: (state) => (sessionId) => {
            if (state.sessions_runs == null)
                return false;
            let session_run = state.getSessionCurrentRun(sessionId);
            let is_running = session_run != null && session_run.status !== SessionStatusEnum.TERMINATED && session_run.status !== SessionStatusEnum.CRASHED && session_run.status !== SessionStatusEnum.STOPPED;
            return is_running;
        },
        getSessionRunsHistory: (state) => (sessionId) => {
            if (state.sessions_runs == null)
                return null;
            return state.sessions_runs.find(session_run => session_run.session === sessionId);
        },
        getSessionCurrentRun: (state) => (sessionId) => {
            if (state.sessions_runs == null)
                return null;
            let session_run = state.sessions_runs.find(session_run => session_run.session === sessionId && session_run.exitStatus == null && session_run.status != SessionStatusEnum.TERMINATED && session_run.status != SessionStatusEnum.CRASHED);
            return session_run;
        },
    ...
    actions: {
        ...
        async stopSession(sessionId) {
            let sessionRun = this.sessions_runs.find(session_run => session_run.session === sessionId && session_run.exitStatus == null);
            if (sessionRun == null) {
                console.error("Session run not found");
                return;
            }
        ...
        async createSessionRun(data) {
            ...
            if (this.sessions_runs == null)
                this.sessions_runs = [];
            if (this.sessions_runs.find(session_run => session_run.id === data.id)) {
