...
export const useDofusMapStore = defineStore("dofusMapStore", {
  ...
  actions: {
    async getWorldsData() {
      if (this.worlds) await this.fetchWorldsData();
      return this.worlds;
    },
    ...
    async getResourcesData() {
      if (this.resources) await this.fetchResourcesData();
    ...
    async getWorldDetails(worldId) {
      if (worldId == null) worldId = this.config.startWorldId;
    ...
    async getTileImage(zoom, x, y) {
      if (
        this.tileExistenceFromCache(zoom, x, y) != null &&
        !this.tileExistenceFromCache(zoom, x, y)
      )
    ...
    async fetchTileImage(zoom, x, y) {
      ...
      if (!this.tiles[zoom]) this.tiles[zoom] = {};
      if (!this.tiles[zoom][x]) this.tiles[zoom][x] = {};
    ...
    getIdTextMapping(language) {
      if (!this.textMapping || !this.textMapping[language])
        this.textMapping[
          language
        ] = require(`@/assets/dofusMap/i18n/${language}.json`);
