<template>
    <q-dialog :model-value="modelValue" @update:modelValue="handleUpdate" persistent>
            ...
            <q-card>
                ...
                <q-card-section class="q-pa-md q-ma-md">
                    <q-form @submit="submitForm">
                        <q-input dense v-model="path.id" label="Name" class="q-mb-md" required />
                        <q-select dense outlined v-model="path.type" :options="pathStore.pathsTypeChoices" label="Type"
                            class="q-mb-md" emit-value required />
                        <div v-if="needStartVertex(path)">
                            <q-input dense v-model="path.startMapId" label="Start Map ID" class="q-mb-md" required />
                            <q-input dense v-model="path.startZoneId" label="Start Zone ID" class="q-mb-md" required />
                        </div>
                        <div v-show="needMapSelection(path)">
                            <q-chip v-for="mapId in path.mapIds" :key="mapId" removable
                                @remove="removeMapFromSelection(mapId)">
                                {{ mapId }}
                            </q-chip>
                            <q-btn flat dense color="primary" class="q-mt-md" label="Open dofusMap"
                                @click="openDofusMap" />
                        </div>
                        <q-card-actions align="center">
                            <q-btn type="submit" flat dense color="primary" class="q-mr-md" label="Confirm">
                                <q-icon name="check" />
                            </q-btn>
                            <q-btn flat dense color="negative" class="q-ml-md" label="Cancel" @click="closeWindow">
                                <q-icon name="close" />
                            </q-btn>
                        </q-card-actions>
                    </q-form>
    ...
    <q-dialog maximized v-model="showDofusMap">
        <DofusMap v-model="showDofusMap" :pathCreatorMode="true" @select="onMapsSelected" :showTitleBar="true"
            height='calc(100vh - 100px)' :initialSelectedResources="initialSelectedResources" :asPathCreator="true" />
    </q-dialog>
</template>
...
export default {
    ...
    methods: {
        ...
        needStartVertex(path) {
            return path.type == PathTypeEnum.RandomSubAreaFarmPath || path.type == PathTypeEnum.RandomAreaFarmPath;
        },
