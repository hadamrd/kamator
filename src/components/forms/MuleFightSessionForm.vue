...
export default {
  ...
  computed: {
    characterSelectOptions() {
      ...
      return this.characters.filter((ch) => {
        return (
          ch.account !== this.leader?.account &&
          ch.account !== this.leader?.account
        );
      });
    },
    ...
    leaderSelectOptions() {
      ...
      return this.characters.filter((ch) => {
        return (
          ch.serverName == this.character.serverName &&
          ch.account !== this.character.account &&
          ch.account !== this.seller?.account
        );
      });
    },
    selectedUnloadType: {
  ...
  watch: {
    character: function (newval, oldval) {
      ...
      if (
        this.leader &&
        (newval.serverName !== this.leader.serverName ||
          newval.account === this.leader.account)
      )
        this.leader = null;
    },
    leader(newLeader, oldLeader) {
      ...
      if (
        this.character &&
        newLeader.account === oldLeader?.account &&
        newLeader.serverName === oldLeader?.serverName
      ) {
        this.character = null;
      }
    },
    unloadType(newVal, oldVal) {
      if (newVal !== UnloadTypeEnum.SELLER) this.seller = null;
    },
  },
};
</script>
