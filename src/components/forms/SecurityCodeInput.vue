    ...
    <q-card class="q-pa-md" style="max-width: 400px; min-width: 300px">
      ...
      <q-card-section>
        <div class="text-subtitle2 q-mb-md">
          A security code has been sent to your email. Please enter it below to
          continue.
        </div>
...
export default {
  ...
  methods: {
    focusNext(index) {
      if (
        this.securityCodeArray[index] &&
        index < this.securityCodeArray.length - 1
      ) {
    ...
    handlePaste(event) {
      ...
      if (paste.length === this.securityCodeArray.length) {
        this.securityCodeArray = paste
          .split("")
          .slice(0, this.securityCodeArray.length);
        this.$nextTick(() => {
