        ...
        <div class="q-pa-md q-ma-md min-width-800px">
            <q-card>
                ...
                <q-card-section class="q-pa-md q-ma-md">
                    <q-input dense v-model="id" label="Session Name" class="q-mb-md" :rules="[validateName]" required />
                    <q-select class="q-mb-md" dense outlined bottom-slots v-model="character" use-input
                        :options="characterSelectOptions" @filter="filterFn" @input="clearFilter"
                        label="main character" option-label="name">
                    </q-select>
                    <q-input type="number" step="1" dense v-model.number="numberOfCovers"
                        label="Number of covers per path" class="q-mb-md" required>
                    </q-input>
                    <q-select dense class="q-mb-md" outlined bottom-slots ref="select" v-model="pathsList" use-input
                        multiple emit-value input-debounce="0" label="select paths" option-label="id"
                        :options="pathStore.paths" @filter="filterFn" @input="clearFilter">
                        <template v-slot:selected-item="{ opt }">
                            <q-chip dense class="q-mr-xs" removable @remove="removeOption(opt)">{{ opt.id }}</q-chip>
                        </template>
                        <template v-slot:append>
                            <q-icon name="close" class="cursor-pointer" icon="clear"
                                @click.stop.prevent="pathsList = []" :disable="pathsList.length < 1"></q-icon>
                        </template>
                    </q-select>
                    ...
                    <q-select dense outlined bottom-slots v-model="seller" :options="characterSelectOptions"
...
export default {
    ...
    computed: {
        ...
        sellerSelectOptions() {
            ...
            return this.characters.filter(ch => {
                return (ch.serverName == this.character.serverName && ch.account !== this.character.account)
            });
        },
        selectedUnloadType: {
            get() {
