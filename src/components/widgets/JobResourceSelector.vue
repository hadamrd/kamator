<template>
  ...
  >
    <template v-slot:selected-item="{ opt }">
      <q-chip dense class="q-mr-xs" removable @remove="removeJobOption(opt)">{{
        getJobNameById(opt)
      }}</q-chip>
    </template>
    <template v-slot:append>
      <q-icon
        ...
        icon="clear"
        @click.stop.prevent="selectedJobs = []"
        :disable="selectedJobs.length < 1"
      ></q-icon>
    </template>
  </q-select>
  <div>
    <div class="resource-grid-label">Select Resources</div>
    <!-- Label for the grid -->
    <div class="grid-container">
      <div
        v-for="jobFilter in jobFilters"
        :key="jobFilter.jobId"
        class="grid-item"
      >
        <div class="text-subtitle2 q-mt-md">
          {{ skills[jobFilter.jobId]?.name }} Resources
        </div>
        <!-- q-select as before -->
        <q-select
          dense
          ...
          bottom-slots
          ref="select"
          v-model="jobFilter.resourcesIds"
          :options="resourceOptions(jobFilter.jobId)"
          option-value="id"
          use-input
          multiple
          emit-value
          ...
          option-label="name"
          @filter="
            (val, update) => filterResources(jobFilter.jobId, val, update)
          "
        ...
        >
          ...
          <template v-slot:selected-item="{ opt }">
            <q-chip
              ...
              class="q-mr-xs"
              removable
              @remove="removeResourceOption(jobFilter.jobId, opt)"
          ...
          <template v-slot:append>
            <q-icon
              ...
              :disable="jobFilter.resourcesIds.length < 1"
            ></q-icon>
          </template>
        </q-select>
      </div>
...
export default {
  ...
  computed: {
    jobsOptions() {
      return this.skillsOptions.filter((skill) => {
        if (this.selectedJobs.includes(skill.id)) return false;

        if (
          this.jobOptionFilterValue &&
          !skill.name
            .toLowerCase()
            .includes(this.jobOptionFilterValue.toLowerCase())
        )
          return false;
  ...
  methods: {
    ...
    getResourceNameById(jobId, resourceId) {
      const resource = this.gameDataStore
        .getJobGatherableResources(jobId)
    ...
    resourceOptions(jobId) {
      ...
      let jobFilter = this.jobFilters.find(
        (jobFilter) => jobFilter.jobId === jobId
      );
      options = options.filter((resource) => {
        if (jobFilter.resourcesIds.includes(resource.id)) return false;
    ...
    updateJobFilters() {
      ...
      this.jobFilters = this.jobFilters.filter((jobFilter) =>
        this.selectedJobs.includes(jobFilter.jobId)
      );

      // Find all selected job IDs that do not have a filter yet
      const jobIdsWithFilters = this.jobFilters.map(
        (jobFilter) => jobFilter.jobId
      );
      const newJobIds = this.selectedJobs.filter(
        (jobId) => !jobIdsWithFilters.includes(jobId)
      );
    ...
    removeJobOption(option) {
      this.selectedJobs = this.selectedJobs.filter(
        (jobId) => jobId !== option.id
      );
    },
    removeResourceOption(jobId, option) {
      const jobFilter = this.jobFilters.find(
        (jobFilter) => jobFilter.jobId === jobId
      );
      jobFilter.resourcesIds = jobFilter.resourcesIds.filter(
        (resource) => resource.id !== option.id
      );
    },
  },
  watch: {
