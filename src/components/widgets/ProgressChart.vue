    ...
    <div v-else class="stats-dashboard">
      ...
      <div v-if="statsSummary" class="stats-summary">
        <div class="summary-card">
          <div class="card-value">{{ formatNumber(statsSummary.totalEarned) }}</div>
...
export default {
  ...
  setup(props) {
    ...
    const calculateRunTime = (sessionRun) => {
      ...
      const endTime = sessionRun.endTime ? new Date(sessionRun.endTime) : new Date();
      return endTime - startTime;
    };

    const statsSummary = computed(() => {
      const stats = webSocketStore.getSessionStats(props.sessionRunId);
      if (!stats || !sessionRun.value || stats.timestamps.length === 0) return null;
    ...
    const chartOptions = computed(() => {
      ...
      // Calculate max values for visible datasets
      let maxKamas = 0;
      let maxCount = 0;
      ...
      return {
        ...
        scales: {
          ...
          y: {
            display: activeDatasets.value.earnedKamas || activeDatasets.value.estimatedKamas,
            beginAtZero: true,
          ...
          y1: {
            display: activeDatasets.value.fights || activeDatasets.value.treasureHunts,
            position: 'right',
            beginAtZero: true,
            ...
            callbacks: {
              label: (context) => {
                ...
                if (context.dataset.yAxisID === 'y') {
                  return `${context.dataset.label}: ${formatNumber(value)} kamas`;
                }
    ...
    const chartData = computed(() => {
      ...
      if (!stats || !sessionRun || stats.timestamps.length === 0) {
        return null;
      }
