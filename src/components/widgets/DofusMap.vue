<template>
    <q-card>
        <q-toolbar shrink v-if="showTitleBar" class="bg-primary glossy dofus-map-title-toolbar">
            <q-toolbar-title class="dofus-map-title-toolbar-title">DofusMap - {{ worldTitle }}</q-toolbar-title>
            <q-space /> <!-- This will push the button to the right -->
            <q-btn icon="close" flat round @click="closeDofusMap" class="dofus-map-title-toolbar-close-button" />
        </q-toolbar>
        <q-toolbar class="dofus-map-menu-toolbar">
            <q-btn-dropdown flat stretch label="Select World" class="dofus-map-menu-toolbar-button">
                <q-list>
                    <q-item v-for="world in store.worlds" :key="world.id" clickable v-close-popup
            ...
            <q-btn flat stretch label="Select Resources" @click="toggleResourcePicker"
                class="dofus-map-menu-toolbar-button" />
        </q-toolbar>
        <div>
            <div ref="mapContainer" id="{{ containerId }}" class="q-ma-xs mapContainer"
                :style="{ height: mapHeight, width: mapWidth, zIndex: zIndex }">
            </div>
            <div v-if="pathCreatorMode" class="selected-maps-section" :style="{ width: mapWidth }">
    ...
    <q-drawer v-model="showResourcePicker" side="right" :width="drawerWidth" style="height:'50%" overlay>
        <ResourcePicker :maxSelections="maxResourceSelections" @finished="toggleResourcePicker"
            @select="onResourcesSelected" :initial-selected-resources="selectedResources" confirmButtonLabel="apply"
            cancelButtonLabel="close" :closeOnApply="false" />
    </q-drawer>
</template>
...
export default {
    ...
    setup(props) {
        ...
        const validatedResources = Array.isArray(props.initialSelectedResources) ? props.initialSelectedResources : [];
    ...
    computed: {
        ...
        hasParent() {
            return this.world && this.world?.parentId && this.world.parentId != this.world.id;
        },
        mapHeight() {
    ...
    watch: {
        ...
        showResourcePicker(newValue, old) {
            ...
            this.$nextTick(() => {
                if (this.map) { // Assuming `mapInstance` is your Leaflet or other map library instance
                    if (newValue) {
                        this.map.resourceLayer.legend.hide();
                    } else {
