<template>
  ...
  <q-card-section>
    <div class="selected-resources-container">
      <div
        class="chip-container q-pa-md"
        :style="{ '--chips-per-row': chipsPerRow }"
      >
        <q-chip
          v-for="selected in selectedResources"
          :key="selected.id"
          removable
    ...
    <div ref="resourceContainer">
      <q-expansion-item
        v-for="(group, skillId) in groupedResources"
        :key="skillId"
        expand-separator
      >
...
export default {
  ...
  methods: {
    ...
    selectFromSearch(resource) {
      if (!this.isSelected(resource)) this.toggleSelect(resource);
