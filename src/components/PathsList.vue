            ...
            <q-item v-for="path in store.paths" :key="path.id" clickable v-ripple>
                ...
                <q-item-section side>
                    <div class="text-grey-8 q-gutter-xs">
                        <q-btn class="gt-xs" dense round flat color="grey"
                            @click="askConfirmDelete(path.id)" icon="delete">
    ...
    <q-dialog v-model="confirmDelete" persistent>
        <q-card>
            <q-card-section class="row items-center">
                ...
                <span class="q-ml-sm">Are you sure ?. This will delete the path definitely!</span>
            </q-card-section>
            <q-card-actions align="right">
                <q-btn flat label="Yes" color="primary" @click="deletePath" v-close-popup />
                <q-btn flat label="Cancel" color="primary" @click="confirmDelete" v-close-popup />
            </q-card-actions>
        </q-card>
    </q-dialog>
</template>
