...
import { getScalingFactor, getTileBoundaries as getTileBoundaries } from './MapTools';
...
export class DofusMapTileLayer extends L.TileLayer {
    ...
    createTile(coords, done) {
        ...
        this.store.getTileImage(z, x, y).then(tileData => {
    ...
    doesTileExist(zoom, x, y) {
        ...
        if (this.world.hasWorldMap && zoom <= this.world.dimensions.maxZoom) {
            ...
            const {
                xBoundaries,
                yBoundaries
            } = getTileBoundaries(zoom, x, y, this.config);
