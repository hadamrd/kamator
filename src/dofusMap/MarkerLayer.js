...
export class MarkerLayer {
    ...
    getMarkerSize() {
        const scaleFactor = getScalingFactor(this.map.getZoom(), this.map.config.maxZoom);
        return [
    ...
    getStaticMarkerIcon(markerType) {
        const iconHTML = `<div class="dofusMap-marker">
                            <img src="${require('@/assets/dofusMap/markerIcons/' + TYPE_ICON[markerType])}" alt="Marker">
                        </div>`;
        return L.divIcon({
            className: 'marker',
            html:iconHTML,
