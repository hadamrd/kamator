...
export function getRectangleOnMapSize(zoom, config) {
  const { imgWidth, imgHeight } = getScaledMapDimensions(zoom, config);
...
export function getTileBoundaries(zoom, x, y, config) {
  const { imgWidth, imgHeight, sideOverlay, bottomOverlay } =
    getScaledMapDimensions(zoom, config);
  const tileOffsetX = getTileOffset(x, config);
  const tileOffsetY = getTileOffset(y, config);
  const xBoundaries = getDimensionBoundaries(
    tileOffsetX,
    sideOverlay,
    imgWidth,
    config
  );
  const yBoundaries = getDimensionBoundaries(
    tileOffsetY,
    bottomOverlay,
    imgHeight,
    config
...
export function getDofusMapCoordinates(latlng, config) {
  let { x, y } = mapXYToDofusXY(latlng, config);
  x = Math.floor(x);
  y = Math.floor(y);
  return {
    x,
