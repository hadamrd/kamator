...
export class MapBorderHighlighter {
  constructor(map) {
    this.mapHighlight = new L.Marker(dofusXYToMapXY(0, 0, map.config), {
      interactive: false,
      icon: L.divIcon({
  ...
  getKnownMapIdMarkerIcon(x, y, mapId) {
    ...
    return new L.divIcon({
      ...
      html: `<div class="icon-container known-maps">
                    ${
                      this.showKnownMapIds
                        ? `<div class="coordinates-label"></div>`
                        : ""
                    }
                </div>`,
      iconAnchor: [0, 0],
  ...
  getSelectedMapMarkerIcon(x, y) {
    ...
    return new L.divIcon({
      ...
      html: `<div class="icon-container">
                    ${
                      shouldDisplayCoords
                        ? `<div class="coordinates-label">${x}, ${y}</div>`
                        : ""
                    }
                </div>`,
      iconAnchor: [0, 0],
  ...
  mouseMoveOnMap(event) {
    ...
    this.mapHighlight.setLatLng(
      dofusXYToMapXY(this.dofusCoords.x, this.dofusCoords.y, this._map.config)
    );
  }
  ...
  updateSize(event) {
    const newIconSize = getRectangleOnMapSize(
      this._map.getZoom(),
      this._map.config
    );
  ...
  getCoordsFromKey(key) {
    ...
    if (parts.length === 2) {
      ...
      throw new Error(
        "Invalid key format. Key must be in the format 'x*y'." +
          `got ${key} instead.`
      );
    }
  }
