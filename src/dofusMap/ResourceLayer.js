...
const resourceIcons = import.meta.glob(
  "/src/assets/dofusMap/resourceSprite/*.png"
);
...
export class ResourceLayer {
  constructor(map) {
    ...
    this.legend = new ResourceLegend({
      position: "topright",
  ...
  async draw(selectedResources) {
    ...
    for (const resource of selectedResources) {
      try {
        ...
        console.error(
          "Error fetching markers for resource:",
          resource.id,
          error
        );
  ...
  createCustomIcon(markerData) {
    return L.divIcon({
      html: `<div class="icon-wrapper">
                    <img src="${resourceIcons(
                      `./${markerData.resource.id}.png`
                    )}" alt="${markerData.resource.name}" />
  ...
  initializeMarkerData(resource, marker) {
    ...
    let existingResource = this.markersData[latlng.lat][
      latlng.lng
    ].resources.find((r) => r.resource.id == resource.id);
  ...
  updateIconStyles(multiplicationFactor) {
    const iconSize = Math.min(
      parseInt(
        markersScaleAmplification * multiplicationFactor +
          this.minMarkerIconSize
      ),
      60
    );
    const iconTextSize = parseInt(
      markersScaleAmplification * multiplicationFactor +
        this.minMarkerIconTextSize
    );
